...
implements RawEntityFactory<RawFilter> {
	...
	public Filter get(long id) throws Exception {
		RawFilter filter = queryForOneFilter(
			"select " + filterRowMapper.selectAll()
				+ " from filter where filter_id = ?", new Object[]{new Long(id)});
		if (filter == null) {
			return null;
		}
		setTags(filter);
	...
	private void setTags(RawFilter filter) throws Exception {
		...
		if (id2class.size() > 0) {
			QueryUtils.setTagsRecursively(id2class, QueryUtils.TAGGING_TARGET_TAG,
				this.jdbcTemplate, getTagRepository());
		}

		// Excludes
		Map<Long, RawTag> id2excludes = QueryUtils.setOnlyParentTags(
			filter.getExcludes(), filter.getId(),
			QueryUtils.TAGGING_TARGET_FILTER_EXCLUDES, this.jdbcTemplate,
			getTagRepository());
		if (id2excludes.size() > 0) {
			QueryUtils.setTagsRecursively(id2excludes, QueryUtils.TAGGING_TARGET_TAG,
				this.jdbcTemplate, getTagRepository());
		}
	}
	...
	public Filter getByName(String name) throws Exception {
		...
		RawFilter filter = queryForOneFilter(
			"select " + filterRowMapper.selectAll()
				+ " from filter where filter_name = ?", new Object[]{name});
		if (filter == null) {
			return null;
		}
		setTags(filter);
	...
	public boolean update(Filter filter) throws BaseDataObsoleteException,
		Exception {
		Assert.Arg.notNull(filter, "filter");
		Assert.require(filter instanceof RawFilter, "filter instanceof RawFilter");
	...
	public Page<Filter> getRecentChanges(int pageSize, int pageIndex)
		...
		List<Filter> results = this.jdbcTemplate.query(sql.toString(),
			filterRowMapper);

		return PageUtils.toPage(results, pageSize, pageIndex,
	...
	protected void doDelete(Filter filter, User user) throws Exception {
		...
		// Delete the fragment
		this.jdbcTemplate.update("delete from filter where filter_id = ?",
			new Object[]{filter.getId()});
	}
	...
	public Map<Long, String> getNames(Set<Long> ids) throws Exception {
		...
		return QueryUtils.getValuesForIds("filter", "filter_name", ids,
			this.jdbcTemplate);
	}
}
